...
class SSAT50Overlay{
 ...
 renderControl(){
  ...
  const head=document.createElement('div');head.className='t50-control__head';const title=document.createElement('strong');title.textContent='APPLICATION DATA';const master=document.createElement('label');master.className='t50-master';const masterInput=document.createElement('input');masterInput.type='checkbox';masterInput.checked=this.state.visible;masterInput.setAttribute('aria-label','Show T50 application overlay');master.append(masterInput,document.createTextNode('Show'));head.append(title,master);
